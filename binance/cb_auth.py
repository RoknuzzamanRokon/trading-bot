...
class CBAuth:
    ...
    def place_market_buy_order(self, product_id, funds):
        """
        Place a market buy order on Coinbase Pro.

        :param product_id: The product ID for the order (e.g., 'BTC-USD').
        :param funds: The amount of funds (quote currency) to use for the buy order.
        """
        path = "/orders"
        method = "POST"
        body = {
        ...
        return self.__call__(method, path, body)
